...
export function UploadInterface({ onUploadSuccess, onProcessStart }: UploadInterfaceProps) {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (files.length > 0) {
      const file = files[0];
      if (file.name.includes('FEC') || file.name.endsWith('.txt') || file.name.endsWith('.csv')) {
        setSelectedFile(file);
        setError(null);
      } else {
        setError('Please select a FEC file (.txt or .csv)');
      }
    }
  };
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          >
            ...
            <p className="text-sm font-medium text-gray-900 mb-1">
              Drag and drop your FEC file here
            </p>
          ...
          <p className="mt-4 text-xs text-gray-500 text-center">
            Supports FEC files in TXT or CSV format with 3+ years of data
          </p>
        </div>
